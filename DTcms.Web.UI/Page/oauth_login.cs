...
namespace DTcms.Web.UI.Page
{
    public partial class oauth_login : Web.UI.BasePage
    {
        ...
        void UserPage_Init(object sender, EventArgs e)
        {
            ...
            {
                HttpContext.Current.Response.Redirect(config.webpath + "error.aspx?msg=" + Utils.UrlEncode("登录失败，用户授权已过期，请重新登录！"));
                return;
            }
            ...
            if (oauthModel != null)
            {
                ...
                if (model == null)
                {
                    HttpContext.Current.Response.Redirect(config.webpath + "error.aspx?msg=" + Utils.UrlEncode("登录失败，授权用户不存在或已被删除！"));
                    return;
                }
