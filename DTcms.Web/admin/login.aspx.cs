...
namespace DTcms.Web.admin
{
    public partial class login : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            Model.users userModel=null;
            if (model == null)
            {
                BLL.users userBll = new BLL.users();
                userModel = userBll.GetModel(userName, DESEncrypt.Encrypt(userPwd), -1);
                if (userModel == null)
                {
                    lblTip.Visible = true;
                ...
                WriteLog(userModel.id,userModel.user_name);
                Utils.WriteCookie("UserId", "DTcms", userModel.id.ToString());
            }
            else
            {
                WriteLog(model.id,model.user_name);
            }
            Session[DTKeys.SESSION_ADMIN_INFO] = model;
            Session.Timeout = 45;
            //写入Cookies
            if (model!=null && cbRememberId.Checked)
            {
        ...
        private static void WriteLog(int userid,string username)
        {
